...
const CreditCardsCategory = () => {
  ...
  const categoryCards = useMemo(() => {
    ...
    return allCards.filter((c) =>
      c.category.some((cat) => cat.toLowerCase() === catKey) ||
      c.tags.some((t) => t.toLowerCase() === catKey)
    );
  }, [catKey, allCards]);

  const { banks, categories: availCats, cardTypes } = useCardMeta(categoryCards);
  const filtered = useMemo(() => sortCards(applyFilters(categoryCards, filters), sortBy), [categoryCards, filters, sortBy]);

  if (!catMeta) return <Navigate to="/credit-cards" replace />;
            ...
            <div className="flex-1 min-w-0">
              {filtered.length === 0 ? (
                <div className="text-center py-20">
                  <p className="text-lg text-muted-foreground">No cards found in this category.</p>
                </div>
              ) : (
...
export default CreditCardsCategory;
