...
const { mainPage, sortOptions } = siteContent.creditCards;
...
const CreditCards = () => {
  ...
  const { banks, categories, cardTypes } = useCardMeta(cardsData);

  useEffect(() => {
    window.scrollTo({ top: 0, behavior: "instant" });
  }, []);

  const filtered = useMemo(() => sortCards(applyFilters(cardsData, filters), sortBy), [filters, sortBy, cardsData]);
  ...
  return (
    <div className="min-h-screen bg-background">
      <LandingNavbar />
      <CreditCardsHero
        badge={mainPage.badge}
        headline={mainPage.headline}
        headlineHighlight={mainPage.headlineHighlight}
          ...
          <div className="flex items-center justify-between mb-8 gap-4">
            <div className="flex items-center gap-3">
              ...
              >
                ...
                Filters
                {activeCount > 0 && (
                  <span className="ml-1 w-5 h-5 rounded-full gradient-bg text-white text-[10px] font-bold flex items-center justify-center">
                    {activeCount}
                  </span>
                )}
              </Button>
